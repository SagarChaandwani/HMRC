/*
========================================================================================
PROJECT:    HMRC Debt Recovery - Automated Risk Segmentation Engine
AUTHOR:     [Sagar Chaandwani]
DATABASE:   SQL Server / Azure SQL

OBJECTIVE:
Transform raw transactional instalment logs into a strategic "Debtor Risk Profile".
This script acts as the Data Warehouse transformation layer feeding the Power BI Command Centre.

KEY TECHNIQUES DEMONSTRATED:
1. Window Functions (LAG) -> To detect "Spiralling" behaviour (Consecutive missed payments).
2. Rolling Aggregates (SUM OVER) -> To calculate live exposure without Group By limits.
3. Ranking (DENSE_RANK) -> To identify the "Top 10" offenders by sector.
4. CTEs (Common Table Expressions) -> For modular, readable, and maintainable code.
5. Handling Nulls/Division -> Robust error handling for financial calculations.
========================================================================================
*/
...
-- Objective: Handle NULL payments and standardise status flags
WITH Clean_Transactions AS (
    SELECT 
        i.Case_ID,
        i.Due_Date,
        i.Amount_Due,
        -- Handle Nulls: If no payment made, set to 0
        ISNULL(i.Amount_Paid, 0) AS Amount_Paid, 
        -- Create a Binary Flag: 1 if Missed/Defaulted, 0 if Paid
        CASE 
...
-- Objective: Look across time to find patterns, not just totals.
Behavior_Metrics AS (
    SELECT 
        ct.Case_ID,
        ct.Amount_Due,
        ...
        -- If current is 1 AND prev is 1, they are Spiralling.
...
-- Objective: Flatten the data to one row per Case/Company for the Dashboard.
...
-- Note: Currency formatting is handled in Power BI, not SQL, to maintain numeric data types for charting.
SELECT 
    Risk_Category,
    Company_Name,
    Sector,
